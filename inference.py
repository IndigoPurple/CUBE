...
from models.util import save_videos_grid, read_video, get_annotation, yaping_annotation
...
if __name__ == "__main__":
    ...
    # print(pil_annotation[0].size, type(pil_annotation[0]))
    ...
    #yaping
    pil_annotation = yaping_annotation(video, annotator)
    # if args.condition == "depth" and controlnet_aux.__version__ == '0.0.1':
    #     pil_annotation = [pil_annot[0] for pil_annot in pil_annotation]
    print(pil_annotation[0].size, type(pil_annotation[0]))
    # Save condition video
    video_cond = [np.array(p).astype(np.uint8) for p in pil_annotation]
    # imageio.mimsave(os.path.join(args.output_path, f"{args.condition}_condition_yaping.mp4"), video_cond, fps=8)
    ...
    # save_videos_grid(sample, f"{args.output_path}/pureEvent_{args.prompt}.mp4")
    # save_videos_grid(sample, f"{args.output_path}/test.mp4")
